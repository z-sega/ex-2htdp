...
; DoorState -> DoorState
; closes an open door over the period of one
; clock-tick.
...
; turns key-event k into an action on world-state s
...
(define (door-action s k)
  (cond
    ...
    [else s]))
; the table suffices because they cover all the
; relevant cases for the DoorState in terms of
; actions.
